        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                if r.success {
                    ...
                    completion(.success(DownloadResult(modelName: name, localPath: path, sizeBytes: r.sizeBytes, sizeMB: Double(r.sizeBytes) / 1_048_576.0)))
                } else {
                    let msg = r.errorMessage.map { String(cString: $0) } ?? "Unknown error"
                    completion(.failure(NSError(domain: "Tabyrus", code: 3, userInfo: [NSLocalizedDescriptionKey: msg])))
                }
            }
        }
    }
