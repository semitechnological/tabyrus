...
impl CompletionResult {
    pub fn new(prefix: String, suggestion: String, confidence: f32, is_ml_based: bool) -> Self {
        Self {
            prefix: c_string(&prefix),
            suggestion: c_string(&suggestion),
            confidence,
            is_ml_based,
        }
    }
...
impl GrammarResult {
    pub fn new(
        ...
        Self {
            original: c_string(&original),
            corrected: c_string(&corrected),
            suggestions: c_string(&json),
            confidence,
        }
    }
}
...
pub unsafe extern "C" fn tabyrus_init() {
    ...
    if !command_exists("mlx_lm.generate") {
        eprintln!("[tabyrus] [WARN] mlx_lm.generate not found on PATH");
    }
...
pub unsafe extern "C" fn tabyrus_set_model(model_name: *const c_char) {
    if let Some(s) = from_c_str(model_name).and_then(|s| parse_model(&s)) {
        if let Ok(mut g) = CURRENT_MODEL
            .get_or_init(|| Mutex::new(CompletionModel::Gemma4))
            .lock()
        {
            *g = s;
        }
...
pub unsafe extern "C" fn tabyrus_set_code_reshape_enabled(enabled: bool) {
    if let Some(f) = CODE_RESHAPE_ENABLED.get() {
        f.store(enabled, Ordering::Relaxed);
    }
}

#[no_mangle]
pub unsafe extern "C" fn tabyrus_set_grammar_enabled(enabled: bool) {
    if let Some(f) = GRAMMAR_ENABLED.get() {
        f.store(enabled, Ordering::Relaxed);
    }
}
...
pub unsafe extern "C" fn tabyrus_get_completion(text: *const c_char) -> *mut c_char {
    let text_str = match from_c_str(text) {
        Some(s) => s,
    ...
    if let Some(result) = ml_complete(&text_str) {
        let json = format!(
            ...
            escape_json(&result.0),
            escape_json(&result.1),
            result.2
        );
...
#[no_mangle]
pub unsafe extern "C" fn tabyrus_normalize_suggestion(
...
fn ml_complete(text: &str) -> Option<(String, String, f32)> {
    ...
    let context: Vec<&str> = if words.len() > 40 {
        words[words.len() - 40..].to_vec()
    } else {
        words.clone()
    };
...
fn fallback_mlx_generate(prompt: &str, max_tokens: usize) -> Option<String> {
    ...
    let output = Command::new("mlx_lm.generate")
        .arg("--model")
...
pub unsafe extern "C" fn tabyrus_free_result(prefix: *const c_char, suggestion: *const c_char) {
    if !prefix.is_null() {
        unsafe {
            drop(CString::from_raw(prefix as *mut c_char));
        }
    }
    if !suggestion.is_null() {
        unsafe {
            drop(CString::from_raw(suggestion as *mut c_char));
        }
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn tabyrus_reshape_code(
    code: *const c_char,
    operation: *const c_char,
) -> *mut CodeReshapeResult {
    let code_str = match from_c_str(code) {
        Some(s) => s,
        None => return std::ptr::null_mut(),
    };
    let op_str = match from_c_str(operation) {
        Some(s) => s,
        None => return std::ptr::null_mut(),
    };
    let enabled = CODE_RESHAPE_ENABLED
        .get()
        .map(|b| b.load(Ordering::Relaxed))
        .unwrap_or(false);
    if !enabled {
    ...
    Box::into_raw(Box::new(CodeReshapeResult::new(
        code_str, reshaped, op_str, confidence,
    )))
}
...
#[no_mangle]
pub unsafe extern "C" fn tabyrus_analyze_clipboard(
    content: *const c_char,
    app_name: *const c_char,
) -> *mut ClipboardAnalysis {
...
pub unsafe extern "C" fn tabyrus_free_clipboard_analysis(analysis: *mut ClipboardAnalysis) {
    ...
    unsafe {
        if !(*analysis).text_type.is_null() {
            drop(CString::from_raw((*analysis).text_type as *mut c_char));
        }
        if !(*analysis).app_name.is_null() {
            drop(CString::from_raw((*analysis).app_name as *mut c_char));
        }
        if !(*analysis).summary.is_null() {
            drop(CString::from_raw((*analysis).summary as *mut c_char));
        }
...
pub unsafe extern "C" fn tabyrus_set_hf_token(token: *const c_char) {
    ...
    if let Ok(s) = unsafe { CStr::from_ptr(token).to_str() } {
        HF_TOKEN.get_or_init(|| Some(s.to_string()));
    }
}

#[no_mangle]
pub unsafe extern "C" fn tabyrus_download_model(
    model_name: *const c_char,
) -> *mut ModelDownloadResult {
    let model_str = match from_c_str(model_name) {
        Some(s) => s,
        None => {
    ...
    let model = match parse_model(&model_str) {
        Some(m) => m,
        None => {
            return Box::into_raw(Box::new(ModelDownloadResult::failure(
                model_str.clone(),
                format!("Unknown: {}", model_str),
            )))
        }
    };
...
pub unsafe extern "C" fn tabyrus_is_model_downloaded(model_name: *const c_char) -> i32 {
    ...
    let model = match parse_model(&name_str) {
        ...
        None => {
            eprintln!(
                "[tabyrus] is_downloaded: parse_model failed for '{}'",
                name_str
            );
            return 0;
        }
...
mod tests {
    ...
    fn init() {
        INIT.call_once(|| unsafe { tabyrus_init() });
    }
    ...
    fn test_grammar_check() {
        init();
        unsafe {
            tabyrus_set_grammar_enabled(true);
        }
    ...
    fn test_code_reshape() {
        init();
        unsafe {
            tabyrus_set_code_reshape_enabled(true);
        }
        ...
        if !r.is_null() {
            unsafe {
                tabyrus_free_code_reshape_result(r);
            }
        }
    }
    ...
    fn test_model_switch() {
        init();
        unsafe {
            tabyrus_set_model(CString::new("zeta-2").unwrap().as_ptr());
        }
    ...
    #[test]
    fn test_command_exists_false_for_missing_binary() {
        assert!(!command_exists("tabyrus-command-that-should-not-exist"));
    }
